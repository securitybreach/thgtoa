...
A solution to this is to use encrypted DNS using DNS over HTTPS[^26] or DNS over TLS[^27] with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^28], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some middle-man from snooping on your requests ... except it might not.

Unfortunately the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave among them) will leak the DNS again through SNI[^29] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> ). Only Firefox as of the writing of this guide supports eSNI (encrypted SNI[^30] also called ECH) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to really hide your DNS requests from a third party. Some countries like Russia[^31] and China[^32] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you won't be able to establish an HTTPS connection with a service if you don't allow them to see what it was.

Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^33] have shown it's still possible to reliably fingerprint and block unwanted requests. Only DNS over Tor was able to demonstrate efficient DNS Privacy in recent studies.

One could also decide to use a Tor Hidden DNS Service or Oblivious DNS over HTTPS (ODoH)[^34] to further increase privacy/anonymity but unfortunately such services are rare and the only reliable one I know provided by Cloudflare (<https://blog.cloudflare.com/welcome-hidden-resolver/>, https://blog.cloudflare.com/oblivious-dns/). I **[personally]{.ul}** think these are viable technical options but are also a moral choice if you want to use them (despite the risk posed by some researchers[^35]).
...
The IMEI (International Mobile Equipment Identity[^36]) and the IMSI (International Mobile Subscriber Identity[^37]) are unique numbers created by mobile phone manufacturers and mobile phone operators.

The IMEI is tied directly to the phone you're using. This number is known and tracked by the mobile phone operators and known by the manufacturers. Every time your phone connects to the mobile network, it will register the IMEI on the network along the IMSI (if a SIM card is inserted but that's not even needed). It's also used by many applications (Banking apps abusing the phone permission on Android for instance[^38]) and smartphone Operating Systems (Android/IOS) for identification of the device[^39]. It is possible but difficult (and not illegal in many jurisdiction[^40]) to change the IMEI on a phone but it's probably easier and cheaper to just find and buy some old (working) Burner phone for a few Euros (this guide is for Germany remember) at a flea market or at some random small shop.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^44].

-   Govt agencies around the world interested in your phone number can and do use[^45] special devices called "IMSI catchers"[^46] like the Stingray[^47] or the Nyxcell[^48]. These devices are able to impersonate (to spoof) a cell phone Antenna and force a specific IMSI (from your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^49] (Man-In-The-Middle Attacks) that will allow them to:
...
For additional safety (if you absolutely have to take your smartphone along with you or if you want to store your Burner phone), you could consider the use of a faraday cage[^50] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^51] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

## Your Wi-Fi MAC address:

The MAC address[^52] is a unique identifier tied to your physical Network Interface (Wired Ethernet or WIFI) and could of course be used to track you if it's not randomized. As it was the case with the IMEI, manufacturers of computers and network cards usually keep logs of their sales (usually including things like: Serial number, IMEI, Mac Addresses, ...) and it's possible again for them to track where and when the computer with the MAC address in question was sold and to whom. Even if you bought it with cash in a supermarket, the supermarket might still have CCTV (or a CCTV just outside that shop) and again the time/date of sale could be used to find out who was there using the Mobile Provider antenna logs at that time (IMEI/IMSI).
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^53].
...
-   IOS/Apple:
    ...
    -   Apple does claim[^54] that they anonymize this data using differential privacy[^55] but you'll have to trust them on that.
...
Geolocation is not only done by using mobile antennas triangulation. It's also done using the WIFIs and Bluetooth devices around you. Operating systems makers like Google (Android[^56]) and Apple (IOS[^57]) maintain a convenient database of most WIFI access points, Bluetooth devices and their location. When your Android smartphone or IPhone is on (and not in Plane mode), it will scan passively (unless you specifically disable this feature in the settings) WIFI access points and Bluetooth devices around you and will be able to geolocate you with more precision than when using a GPS.
...
Your metadata is all the information about your activities without the actual content of those activities. For instance it's like knowing you had a call from an oncologist before then calling your family and friends successively. You don't know what was said during the conversation but you can guess what it was just from the "metadata"[^58].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^59]/IOS), Browsers, Apps, Websites. Odds are there are several companies knowing exactly where you are at any time[^60] because of your smartphone[^61].

This location data has been used in many judicial cases[^62] already as part of "geofence warrants" [^63] that allows law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time.
...
The question is: Is there someone somewhere that would possibly have both pieces of information available[^64] for correlation in a convenient database?

Have you heard of Edward Snowden[^65]? Now is the time to google him and read his book[^66]. I recommend reading about XKEYSCORE[^67]'[^68], MUSCULAR[^69] and PRISM[^70].

"We kill people based on Metadata"[^71]
...
All of this data is very likely being transmitted, processed and stored (unencrypted[^72]) by various third parties.
...
-   Your Fitness Devices and Apps? (Strava[^73]'[^74], Fitbit[^75], Garmin, Polar[^76], ...)[^77]

-   Your Smart Speaker? (Amazon Alexa[^78], Google Echo, Apple Homepod ...).
...
-   IPhones and IPads (IOS 13 and above)[^79]'[^80].

-   Samsung Phones (Android 10 and above)[^81].

-   MacBooks (MacOS 10.15 and above)[^82].

Such devices will continue to broadcast identity information to nearby devices even when off using Bluetooth Low-Energy[^83]. They don't have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^84].
...
RFID stands for Radio-frequency identification[^85], it's the technology used for instance for contactless payments and various identification systems. Of course your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^86]. As with everything else, such capabilities can be used for tracking by various actors.
...
The only way to mitigate this problem is to have no RFID tags on you or to shield them again using a type of faraday cage. You could also use specialized wallets/pouches that specifically block RFID communications. Many of those are now made by well-known brands such as Samsonite[^87].

## Your Files Properties/Metadata:

This can be obvious to many but not to all. Most files have metadata attached to them. A good example are pictures which store EXIF[^88] information which can contain a lot of information such as GPS coordinates, which camera/phone model took it and when it was taken precisely. While this information might not directly give out who you are, it could tell exactly where you were at a certain moment which could allow others to use different sources to find you (CCTV or other footage taken at the same place at the same time during a protest for instance). It's important that you verify any file you would put on those platforms for any properties that might contain any information that might lead back to you.

Here is an example of EXIF data that could be on a picture:![][4]
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^89]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^90] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It's able to do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][7]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^91] a bomb threat hoax at Harvard University. It was also used to de-anonymize
...
Be aware again that this might not be enough against a motivated global adversary[^92] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^93] and it's use could be considered malicious by some[^94].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^95]. Most crypto currencies can be tracked accurately through various methods[^96].

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You'll have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^97] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy in mind like Monero but even those can be de-anonymized to some extent[^98][^99].

Even if you use Mixers or Tumblers (services that specialize in anonymizing crypto currencies by "mixing them"), keep in mind this is only obfuscation and not actual anonymity[^100].

## Exploits in your apps:

So you're using Tor Browser or Brave Browser with a Tor Tab. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^101] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^102] case in 2013 where the FBI inserted malware[^103] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users.
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^104] without being able to access identifying information.

## Your CPU:

All modern CPUs[^105] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^106] and the AMD Platform Security Processor[^107].
...
These have already been affected by several security vulnerabilities in the past[^108] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^109].

There are some not so easy ways[^110] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings.
...
There are specialized commercial forensics solutions available (Magnet Axiom[^111], Cellebrite Cloud[^112]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write, the way you behave. The way you click. The way you browse. The fonts you use on your browser[^113]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used used in CAPTCHAS[^188] services to verify that you are "human" and can be used to fingerprint a user.
...
Even if you're not looking at the camera, they can still figure out who you are[^123] and even make out your emotions[^124].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^125]'[^126]'[^127]'[^128]'[^129].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provider by private companies such as the Israeli AnyVision[^130]) in their CCTV networks to look for "persons of interest"[^131]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^132] including possibly identifying ethnic minorities[^133].
...
We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^134], Person of Interest[^135] and Minority Report[^136]).

At this time, there are a few steps[^137] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^138].
...
Phishing[^139] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^49] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^140]. These techniques are widely used by law enforcement all over the world and should be considered.
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^141] or a national security letter[^142]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^143].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^144].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^145].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^146].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^147].

Some providers have implemented the use of a Warrant Canary[^148] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries(see the Crypto AG story[^149] and Omnisec story[^150]).
...
There are many advanced techniques that can be used by skilled adversaries[^151] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel). I recommend having a look if you're curious.
...
TAILS[^153] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
Ideally (but not mandatory[^154]), you should get a dedicated that won't be tied to you in any easy way laptop (paid with cash anonymously as previously mentioned for the phone and the SIM card).
...
-   Enable BIOS/UEFI password and use a long passphrase[^155] instead of a password if you can.
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^156] (with pictures).
...
This route will make extensive use of Virtual Machines[^157], they'll require a host OS to run the Virtualization software. You have two choices in this part of the guide:

-   Your Linux distribution of choice (excluding Qubes)

-   Windows 10 Home edition

Linux is not necessarily the better choice for privacy depending on your threat model. This is because using Windows will allow us to use a privacy feature called Plausible Deniability[^158].
...
A (nice) adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not not incriminate yourself") but some countries are implementing laws[^159] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Veracrypt[^162] is the software we will use for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
The second catch is the "Cold Boot attack"[^165] discussed briefly earlier in this guide. This is trickier than the Evil Maid Attack as it requires them to come into possession of your laptop while you're actively using the hidden OS/data or very shortly afterward.

The principle is simple, as shown in this video[^166], they could theoretically quickly boot your laptop on a special USB key that would copy the content of the RAM (the memory) of the laptop after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance). The memory won't decay and they could be able to copy its content for analysis. This memory dump should contain the key to decrypt your hard drive/data. To mitigate this you have to apply a few principles:

-   Encrypt the memory with a Veracrypt option[^167] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^168].
...
There have been some forensics studies[^169] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^170] and by the maintainer of Veracrypt[^171].
...
A very good example of this are the Windows "ShellBags" that are stored within the Windows Registry storing various histories of accessed files/folders[^172].

Forensics can[^170] and will[^172] use all those leaks[^168] to prove the existence of hidden data and defeat your attempts at using plausible deniability.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^173] Operations[^174]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^155] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^181] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In the Whonix route, we will make extensive use of the free Oracle Virtualbox[^182] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like KVM in Linux, feel free to do so).
...
This route will use Virtualization and Whonix[^183] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
You will need something to store your data (logins/passwords, identities and TOTP[^186] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^187] authentication with the authenticator feature.
...
Captcha[^188] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^189] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^190]) and Cloudflare (hCaptcha[^191]). hCaptcha is used on 15% of the internet by their own metrics[^192].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^193]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^194].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^195].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^197].
...
Browser and Device[^198] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^199]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^200] and unfortunately sometimes required.
...
Some platforms/apps will require you to take a live picture of yourself either doing something (a wink, holding an arm up ...) or showing a custom piece of information (a hand written text, a passport or ID) within the picture. Sometimes the platform/app will require several pictures to increase their certainty.![][16]

This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^201]. Unfortunately this method is now also sometimes being used on Facebook[^202] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^199] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^203].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^204].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^205] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^208] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^209] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
They're recommended by privacytools.io[^211]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^212]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form [^213], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
-   Use an Android Emulator (Youwave[^214], Bluestacks[^215] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^168]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   The Windows USN journal which stores plenty of information[^216].

-   The Windows System Resource Usage Monitor (SRUM)[^217].

-   Various histories of various programs (such as the recent lists).

-   The free (unallocated) space of your hard drive[^218].
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   Use the Wizard:
        ...
        -   Select Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
            ...
            -   If you have an SSD drive, Select SSD with Trim[^173] + 1 pass zero (just to be sure as mentioned earlier, Trim itself should be enough[^176])

            -   If you have an HDD drive, Select HDD with at least 1 pass.

### Shellbags:

As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^172] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
There are several utilities that are recommend (like the old unmaintained DBAN[^219]) for this but personally, I will recommend the use of ShredOS.
...
As written earlier in this guide and as noted by Privacytools.io[^220], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
[^4]: This World of Ours, James Mickens <https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf>

[^5]: XKCD, Security <https://xkcd.com/538/>

[^6]: Wikipedia, Threat Model <https://en.wikipedia.org/wiki/Threat_model>

[^7]: Bellingcat <https://www.bellingcat.com/>

[^8]: Wikipedia, Doxing <https://en.wikipedia.org/wiki/Doxing>

[^9]: YouTube, Internet Historian, The Bikelock Fugitive of Berkeley <https://www.youtube.com/watch?v=muoR8Td44UE>

[^10]: BBC News, Tor Mirror <https://www.bbc.com/news/technology-50150981>

[^11]: GitHub, Real World Onion websites <https://github.com/alecmuffett/real-world-onion-sites>

[^12]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL>

[^13]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf>. Section 13, Article 6, "The service provider must enable the use of telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^14]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil>

[^15]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^16]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^17]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address>

[^18]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention>

[^19]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network>

[^20]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)>

[^21]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System>

[^22]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking>

[^23]: CensoredPlanet <https://censoredplanet.org/>

[^24]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf>

[^25]: Labzilla.io, Your Smart TV is probably ignoring your PiHole <https://labzilla.io/blog/force-dns-pihole>

[^26]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS>

[^27]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS>

[^28]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole>

[^29]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication>

[^30]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^31]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^32]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^33]: KUL, Encrypted DNS=⇒Privacy?A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^34]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^35]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^36]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^37]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^38]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^39]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^40]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>
...
[^43]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^44]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^45]: The Incercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^46]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^47]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^48]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^49]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^50]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^51]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^52]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^53]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^54]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^55]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^56]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^57]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^58]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/>)

[^59]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault>

[^60]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/>

[^61]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html>

[^62]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/>

[^63]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant>

[^64]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A>

[^65]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden>

[^66]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>

[^67]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore>

[^68]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html>

[^69]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)>

[^70]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)>

[^71]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/>

[^72]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT>

[^73]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^74]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^75]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^76]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^77]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^78]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>

[^79]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^80]: Apple Support <https://support.apple.com/en-us/HT210515>

[^81]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^82]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^83]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^84]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^85]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^86]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^87]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^88]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^89]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^90]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^91]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^92]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^93]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^94]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^95]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>
...
[^97]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^98]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^99]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^100]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^101]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^102]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^103]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^104]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^105]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^106]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^107]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^108]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^109]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^110]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^111]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^112]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^113]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^114]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>
...
[^116]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^117]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^118]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^119]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^120]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>
...
[^122]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^123]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^124]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^125]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^126]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^127]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^128]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^129]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^130]: Anyvision <https://www.anyvision.co/>

[^131]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^132]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^133]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^134]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^135]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^136]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^137]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^138]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^139]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^140]: Grayshirt, <https://www.grayshift.com/>

[^141]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^142]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^143]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^144]: Pcmag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^145]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^146]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^147]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^148]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^149]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^150]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^151]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^152]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^153]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^154]: It's not mandatory because this guide will help you harden your laptop as much as possible to prevent online leaks through various means. There will be several lines of defense standing between your online identities and yourself that should prevent most adversaries from de-anonymizing you besides state/global actors with considerable resources.

[^155]: XKCD, Password Strength <https://xkcd.com/936/>

[^156]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^157]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^158]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^159]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^160]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^161]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^162]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^163]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^164]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^165]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^166]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^167]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^168]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^169]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^170]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^171]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^172]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^173]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^174]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^175]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^176]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^177]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^178]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^179]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^180]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^181]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^182]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^183]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^184]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^185]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^186]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^187]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^188]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^189]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^190]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^191]: hCaptcha <https://www.hcaptcha.com/>

[^192]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^193]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^194]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^195]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^196]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^197]: Privacy International,

    Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^198]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^199]: Developers Google Blog,
...
[^200]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^201]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^202]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^203]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^204]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^205]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^206]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^207]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^208]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^209]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^210]: Jumio, ID verification features <https://www.jumio.com/features/>

[^211]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^212]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^213]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^214]: Youwave, <https://youwave.com/>

[^215]: Bluestacks, <https://www.bluestacks.com/>

[^216]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^217]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^218]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^219]: DBAN, <https://dban.org/>

[^220]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>
  ...
  [Advanced targeted techniques:]: #advanced-targeted-techniques
  [Notes:]: #notes
